...
function buildSystemPrompt(text) {
  // 短文本（单词/短语）：简洁分析
  if (text.length <= 10) {
    return getCustomPrompt('short') || `你是日语语法专家。分析日语文本，返回JSON（不要其他文字）：
...
conjugation：有变形的词才列，form填写该词在句中的变形名称。`;
  ...
  // 长文本（句子）：完整分析
  return getCustomPrompt('long') || `你是日语语法专家。仔细分析日语文本，返回JSON（不要其他文字）：
{
...
function getCustomPrompt(type) {
  if (!config || !config.customPrompts) return null;
  return config.customPrompts[type] || null;
}
...
function getActivePrompt(type) {
  ...
  return {
    isCustom: !!custom,
    content: custom || getDefaultPrompt(type)
  };
}
 ...
 * 保存自定义提示词
 */
function saveCustomPrompt(type, prompt) {
  if (!config) loadConfig();
  
  if (!config.customPrompts) {
    config.customPrompts = { short: null, long: null };
  }
  
  // 空字符串或与默认相同时，设为 null（使用默认）
  const trimmed = (prompt || '').trim();
  if (!trimmed || trimmed === getDefaultPrompt(type)) {
    config.customPrompts[type] = null;
  } else {
    config.customPrompts[type] = trimmed;
  }
...
function resetPrompt(type) {
  return saveCustomPrompt(type, null);
}
